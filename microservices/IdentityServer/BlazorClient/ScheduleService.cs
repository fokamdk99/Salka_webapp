...
namespace BlazorClient
{
    public class ScheduleService : IHttpService
    {
        ...
        {
            var reservations = await httpClient.GetFromJsonAsync<List<ReservationModel>>("https://localhost:10001/Schedule/reservationmodels");
            return reservations;
        }
        ...
        public async Task PostNewReservationAsync(ReservationModel reservationModel)
        {
            ...
            await httpClient.PostAsync("https://localhost:10001/Schedule/", jsonString);
        }

        public async Task<List<ReservationModel>> GetReservationsByDateScope()
        {
            var reservations = await httpClient.GetFromJsonAsync<List<ReservationModel>>("https://localhost:10001/Schedule/reservationsbydate");
            return reservations;
        }
    }
}
